...
class GraphQueryAgent:
    ...
    def _leases_by_broker(self, broker_name=None, **_):
        """
        Get all leases handled by a specific broker.
        """
        # Find the broker node by name
        broker = next((n for n, d in self._broker_nodes() if d.get("name") == broker_name), None)
        if not broker:
            return f"Broker {broker_name} not found"

        # Find all leases connected to the broker
        leases = [l for l in self.graph.predecessors(broker) if self.graph.nodes[l].get("type") == "Lease"]
        return leases or f"No leases found for broker {broker_name}"
    ...
    def _total_gci_by_broker(self, **_):
        """
        Calculate the total GCI for each broker.
        """
        broker_gci = {}
        for lease, data in self._lease_nodes():
            gci = self._to_float(data.get("gci"))
            if gci is not None:
                brokers = [b for b in self.graph.successors(lease) if self.graph.nodes[b].get("type") == "Broker"]
                for broker in brokers:
                    broker_gci[broker] = broker_gci.get(broker, 0) + gci

        # Format the result as a dictionary with broker names and total GCI
        result = {
            self.graph.nodes[broker].get("name", broker): total_gci
            for broker, total_gci in broker_gci.items()
        }
        return result if result else "No GCI data available for brokers."

    def _average_rent_by_broker(self, **_):
        """
        Calculate the average annual rent for each broker.
        """
        broker_rent = {}
        broker_count = {}

        for lease, data in self._lease_nodes():
            rent = self._to_float(data.get("annual_rent"))
            if rent is not None:
                brokers = [b for b in self.graph.successors(lease) if self.graph.nodes[b].get("type") == "Broker"]
                for broker in brokers:
                    broker_rent[broker] = broker_rent.get(broker, 0) + rent
                    broker_count[broker] = broker_count.get(broker, 0) + 1

        # Calculate the average rent for each broker
        result = {
            self.graph.nodes[broker].get("name", broker): broker_rent[broker] / broker_count[broker]
            for broker in broker_rent
        }
        return result if result else "No rent data available for brokers."
